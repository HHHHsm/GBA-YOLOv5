...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, DCNConv,ghostC3,GhostConv,ECA, GhostCond,SE,GAM_Attention,Bottleneck, FasterNeXt,GhostBottleneck, SOCA,BottleneckDCN, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,C3, C3TR, CoordAtt, C3DCN,C3STR,CoT3,GAMAttention,CBAM,CrissCrossAttention]:
            c1, c2 = ch[f], args[0]
            if c2 != no:  # if not outputeqqeqe
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in [BottleneckCSP, C3, C3STR,C3TR,ECA, C3DCN]:
